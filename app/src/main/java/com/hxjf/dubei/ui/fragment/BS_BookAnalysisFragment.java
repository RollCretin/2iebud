...
public class BS_BookAnalysisFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    private static final int REFRESH_COMPLETE = 100;
    private static final String TAG = "BS_BookAnalysisFragment";
    ...
    private void init() {
        ...
        Call<BSBookListBean> bsChaiduListCall = ReaderRetroift.getInstance(getContext()).getApi().BSChaiduListCall();
        EnhancedCall<BSBookListBean> bsBookListBeanEnhancedCall = new EnhancedCall<>(bsChaiduListCall);
        bsBookListBeanEnhancedCall.dataClassName(BSBookListBean.class).enqueue(new EnhancedCallback<BSBookListBean>() {
            ...
            public void onGetCache(BSBookListBean bsBookListBean) {
                Toast.makeText(getActivity(), "请检查网络..", Toast.LENGTH_LONG).show();
                if (bsBookListBean != null && bsBookListBean.getResponseData() != null) {
        ...
        if (responseDataList.size() == 0) {
            ...
            bsGvBook.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    if (responseData.getBookInfo().getStatus() == 0) {
...
// TODO: handle exception
    ...
    private void read() {
        ...
        if (!file.exists()) {
            ...
            //下载
            download();
           /* DUtil.init(getActivity())
                    .url(ReaderRetroift.BASE_URL + "/download" + responseData.getBookInfo().getPath())
                    .path(Environment.getExternalStorageDirectory().getPath())
                    .name(responseData.getBookInfo().getPath())
                    .childTaskCount(3)
                    .build()
                    .start(callback);*/
        } else {
    ...
    private void download() {
        downloaddialog = new DownloadProgressDialog(getContext());
    ...
    private void deletechaidu(final int position) {

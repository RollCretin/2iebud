...
class SelectionPopup extends PopupPanel implements View.OnClickListener {
	...
	private static final String TAG = "SelectionPopup";
	...
	public void createControlPanel(FBReader activity, RelativeLayout root) {
		...
		((SelectionPopup) fbReaderApp.getPopupById(SelectionPopup.ID)).move(fbReaderApp.getTextView().getSelectionStartY(),fbReaderApp.getTextView().getSelectionEndY());
...
//		setupButton(R.id.selection_panel_translate, resource.getResource("translate").getValue());
	...
	public void move(int selectionStartY, int selectionEndY) {
		...
		if (diffTop > diffBottom) {
			...
			layoutParams.setMargins(0,selectionEndY,0,0);
		} else {
			layoutParams.addRule(RelativeLayout.ALIGN_PARENT_TOP);
			layoutParams.setMargins(0,selectionStartY - myWindow.getHeight(),0,0);
		}

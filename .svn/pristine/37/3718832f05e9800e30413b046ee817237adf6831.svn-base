...
public class  EditBookmarkActivity extends Activity implements IBookCollection.Listener<Book> {
	...
	public void onCreate(Bundle bundle) {
		...
		addTab(tabHost, "笔记", R.id.edit_bookmark_content_text);
...
//		addTab(tabHost, "delete", R.id.edit_bookmark_content_delete);
		...
		String note = myBookmark.getText().toString();
		String newNote = note.replaceFirst("^#*", "");
		editor.setText(newNote);
		...
		saveTextButton.setText("保存笔记");
		saveTextButton.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				myCollection.bindToService(EditBookmarkActivity.this, new Runnable() {
					public void run() {
						String originalText = myBookmark.getText().toString();
						if (!originalText.equals(editor.getText().toString())){
		...
		editor.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence sequence, int start, int before, int count) {
				...
				boolean bookmarkchange =  originalText.equals(editor.getText().toString());
				saveTextButton.setEnabled(!bookmarkchange);
			}
